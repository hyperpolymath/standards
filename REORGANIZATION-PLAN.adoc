...
> **Status note (2026-09-19):** this plan predates the standards-map
> measurement that settled the `rhodium-standard-repositories/` question:
> the upstream repo 404s and 8 of its 10 satellites have no external home,
> so the tree is the LAST COPY and is archived in place ("do not delete" -
> see standards-map.toml). Plan passages proposing to delete, move, or
> re-home that tree are superseded; the passage about the spec extraction
> to `0-canon/rsr/` was executed 2026-09-17.
...
`+2-protocols/0-ai-gatekeeper/contractiles/k9/template-*.k9.ncl+` (3 copies
per subdir) -
`+2-protocols/0-ai-gatekeeper/*/contractiles/k9/template-*.k9.ncl+`
(multiple copies)
...
*Current:* Mixed in `+.machine_readable/descriptiles/+` with other files
