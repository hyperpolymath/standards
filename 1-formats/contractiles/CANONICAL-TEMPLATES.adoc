...
This document defines the estate-level meaning of the canonical `1-formats/contractiles/`
...
| Adjust
| `1-formats/contractiles/adjust/Adjustfile.a2ml`
...
| Bust
| `1-formats/contractiles/bust/Bustfile.a2ml`
...
* Enforced by three coupled checks, all keyed on `glama.json`:
  ...
  `1-formats/contractiles/must/Mustfile` (`readme-md-glama-carveout` — requires the
  `README.md` when `glama.json` is present), and
  `1-formats/contractiles/dust/Dustfile` (`preserve: glama-readme` — forbids cleanup
  from deleting it).
====
