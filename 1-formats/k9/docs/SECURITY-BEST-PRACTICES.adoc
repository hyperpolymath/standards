...
- link:../SECURITY.md[3-practice/SECURITY.md] - Vulnerability reporting policy
...
**⚠️ CRITICAL: Trusting a key grants full system access for Hunt components.**
...
# Must see: "✓ Valid signature from [trusted key]"
...
4. If potentially malicious, report to 3-practice/SECURITY.md contacts
...
```nickel
# ❌ Base64 encoding (hiding commands)
...
```nickel
# ❌ Unless component purpose is networking
...
```nickel
# ✅ Good Yard use - controlled file output
K9!
leash = 'Yard
...
# ✅ ALWAYS sandbox when possible (see below)
```
...
**⚠️ CRITICAL for Hunt components from external sources.**
...
⚠️  DRY-RUN MODE - Nothing was actually executed
To run for real: ./must deploy component.k9.ncl
```

=== Dry-Run Limitations

**⚠️ Dry-run is NOT perfect:**
...
**⚠️ NEVER run K9 components as `root` unless absolutely required.**
...
if [ "$1" = "--allow-root" ] && [ "$(id -u)" -eq 0 ]; then
    read -p "⚠️  You are running K9 as ROOT. Type 'I understand the risks' to continue: " response
...
4. **Report to K9 maintainers**
   ...
   - GitHub: https://github.com/hyperpolymath/standards/tree/main/k9-svc/security/advisories/new
...
if [ "$(id -u)" -eq 0 ]; then
    echo "❌ ERROR: k9-safe refuses to run as root."
...
if ! openssl pkey -pubin -in "$KEY_FILE" -text -noout | grep -q "ED25519"; then
    echo "❌ Not a valid Ed25519 public key"
    exit 1
fi
...
- **3-practice/SECURITY.md** - Vulnerability reporting policy
