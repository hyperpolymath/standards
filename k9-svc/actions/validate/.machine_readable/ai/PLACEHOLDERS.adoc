...
sed -i 's/k9-validate-action/my-project/g' $(grep -rl 'k9-validate-action' .)
sed -i 's/STANDARDS/MY_PROJECT/g' $(grep -rl 'STANDARDS' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `k9-validate-action` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `` | One-line description | `A tool for X` | flake.nix |
| `STANDARDS` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `k9-validate-action` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `https://github.com/hyperpolymath/k9-validate-action` | Project website | `https://example.org` | SECURITY.md |
