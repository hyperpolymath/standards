...
sed -i 's/Hyperpolymath Standards/my-project/g' $(grep -rl 'Hyperpolymath Standards' .)
sed -i 's/STANDARDS/MY_PROJECT/g' $(grep -rl 'STANDARDS' .)
sed -i 's/{{project}}/my_project/g' $(grep -rl '{{project}}' .)
sed -i 's/standards/my-project/g' $(grep -rl 'standards' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Hyperpolymath Standards` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
...
| `standards` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
