...
// NOTE: extracted to metadatastician/consent-aware-web on 2026-08-07; the
// observation below describes the tree as it stood before that.
...
**SOURCE STATUS (verified 2026-09-17, standards#645 + PR #835):** the scaffold
hunt is resolved — every candidate was checked before touching anything.

* rsr-template-repo: flipped on 2026-05-22 (`04997b9`, "chore: migrate license
  from PMPL-1.0-or-later to MPL-2.0") — THREE MONTHS before the residue was
  measured; today's residue was minted from the older vintage (`2283f12`).
* scaffoldia: NOT an emitter — its 7 PMPL hits are all
  `machine-readable-design/harvested-registry/**.ncl` package-registry DATA
  records; editing them would falsify harvested facts.
* llm-warmup-* templates: 404, gone — cannot emit.
* standards-side remnant (a second, dormant vector): the v1
  `STATE.a2ml.template:46` comment literally told mints "Almost always
  PMPL-1.0-or-later" while shipping `license = "MPL-2.0"`. Fixed at source
  (PR #835, merged 2026-09-17).
* axel-protocol's three `trust.txt` copies: VERIFIED AUTHORED and consistent
  (six normative PMPL declarations in the same directory) — leave, exactly as
  §3 says, and reconcile at the corpus level when axel-protocol's own licence
  story is settled.

So: every emitter is now either fixed, extinct, or excluded. Under the §7a
strategy the remaining work is the §1/§3-excluding re-propagation + residue
clean — which is owner-run, exactly as planned.
