...
key is never trusted merely because `ssh-keyscan` returned it.
...
GitLab, Bitbucket, Codeberg, and Disroot have published fingerprints pinned
...
* https://disroot.org/services/git[Disroot]

SourceHut requires `SOURCEHUT_SSH_FINGERPRINT`. A custom Gitea mirror requires
...
`scripts/tests/science-ci-security-test.sh` exercises every actual SSH
