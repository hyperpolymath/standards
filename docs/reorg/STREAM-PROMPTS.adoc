...
* *Owner decisions captured 2026-06-03:* A2ML v1.1 ships profile mechanism + base
  ...
  profiles declared in-document via `@profile(id)`; the descriptiles family is re-expressed
...
== Stream 1 — A2ML + K9-SVC foundation (+ descriptiles)

[source,text]
----
Work in the hyperpolymath/standards repo (a2ml/, 1-formats/k9/, .machine_readable/).
...
- descriptiles family becomes A2ML profiles: a2ml/state, a2ml/meta, a2ml/ecosystem,
...
descriptiles (.machine_readable/):
7. Author the seven profile definitions (a2ml/state ... a2ml/anchor).
8. Add @profile directives to the existing descriptiles files so they validate through the
   mechanism. Add a hypatia rule that flags a descriptiles file whose content drifts from its
   declared profile.
...
1. Generalise the existing 1-formats/SATELLITES.a2ml into (or alongside) a REGISTRY that can
...
- TOPOLOGY.adoc + ROADMAP.adoc frozen at 2026-04-04; TOPOLOGY shows integration layer
  at 0% while claiming 80% overall.
- EXPLAINME Evidence table lists 2-protocols/groove/ and palimpsest-license/ as local
...
- Competing entry docs: README.adoc, EXPLAINME.adoc, TOPOLOGY.adoc, 0-AI-MANIFEST.a2ml,
  QUICKSTART-{DEV,USER,MAINTAINER}.adoc, llm-warmup-*.md, 3-practice/ai-instruction/.
...
5. Make TOPOLOGY.adoc DERIVED/auto-refreshed (from STATE.a2ml / registry) rather than a
