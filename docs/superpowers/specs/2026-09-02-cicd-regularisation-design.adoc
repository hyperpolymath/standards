...
[start=0]
. Commit this spec to `standards/docs/superpowers/specs/2026-09-02-cicd-regularisation-design.adoc`; write the R1–R7 rulings to memory; file the owner-action issue (O1–O3).
. *Canonical artefacts in `standards`*: `rulesets/base.json`, `rulesets/base-classic.json`, `settings/repo.json`, `autolinks/{base,rust,proof,julia,elixir}.json`, tier doc update (`docs/CICD-SIGNAL-DISCIPLINE.adoc`), MUST list.
. *Reusables*: remake `governance-reusable.yml` as N jobs (fold §5.2 policy family + workflow-linter + dogfood + SPDX); add `actions-lock-verify` + Dependabot lock-sync job; delete `deno-ci-reusable.yml`; create `julia-ci`, `bun-ci`, `ada-ci`, `zig-ci`, `pages`, `release` reusables. Each new job ships with a planted-positive test in `standards` CI.
. *Applier/verifier*: extend `branch-protection-apply.sh`; write `settings-drift.sh`; retire the seven other appliers (delete, with a pointer).
. *Pilot on 6 repos* (2 P-pub, 1 P-priv, 2 P-org, 1 proof repo): apply, run verifier, plant a positive per GATE, record the first real red.
. *Sweep* (bot-driven, alphabetical, resumable, with a stall detector — the 08-28 sweep died silently mid-alphabet): delete DROP files, replace inline with thin callers, fix 3 broken refs, pin `@main` refs, regen lockfiles, apply settings/ruleset/autolinks, remove app installs. Track in one issue with per-repo checkboxes.
. *`.github` repos* (§7.5) + `workflow-templates/`.
. *Owner-gated moves* (§8) after O1.
