...
| `push-email-notify.yml` | 341 | 0 green, **325 disabled**, 18 skipped, 11 sf | **REMAKE · dormant** (owner ruling 2026-09-02: "replace it everywhere"). Canonical = `rsr-template-repo` `.github/workflows/push-email-notify.yml` (re-landed 2026-08-31, #54: `vars.PUSH_EMAIL_ENABLED == 'true'` gate, `branches: ['**']`, 5-min timeout). Pin form follows the repo's regime, because inline SHA and `actions.lock` are mutually exclusive (measured: 14 startup_failures): **lock repos (135)** take the canonical verbatim, `uses: hyperpolymath/smtp-notify-action@v0.1.0 # NOSONAR — pin authority is actions.lock (sha1-1b3b752d39a4fe4c0f28f10905e4608789d3e050)`, lock regenerated in the same PR (§6.4); **no-lock repos (202)** take the same body with `uses: hyperpolymath/smtp-notify-action@1b3b752d39a4fe4c0f28f10905e4608789d3e050 # v0.1.0` and without the "managed by gh actions-lock" header. Census 2026-09-02 (per-repo API): 337 repos carry the file, 329 hyperpolymath + 8 metadatastician. The 325 disabled copies stay disabled until the owner re-arms them (no SMTP secrets exist). Sweep engine = first run of step 5 |
