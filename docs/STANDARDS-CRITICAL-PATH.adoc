...
Separately, **76 files sit at a bare-root `1-formats/contractiles/must/`** (no
...
**Resolved 2026-09-02 — the HEAD side is correct.** `source_hash` is NOT a hash
...
`f9d32937c6d0541c82ada3d7c42cb7347fd4bbb3daa167b51b0cc420824918cc` — exactly the
...
drift recurs and blocks every PR — needs a regen step or pre-commit hook") was
...
| 10 | Third layout root: `rsr-template-repo` uses `machine-readable/` (hyphen, no dot), matched by **1 file estate-wide**. Correct it to `.machine_readable/`? — **RESOLVED: yes.** Owner ruling 2026-09-17, on the census (48 dotted vs 9 hyphenated), which makes the spine the minority. | Executed
|===
...
| — | Delete `standards`' stale root-level `1-formats/contractiles/` — extension-less `Mustfile`, `Dustfile`, superseded by `.machine_readable/` | Flag for owner; deletion is cheap but irreversible
...
. **Fix `rsr-template-repo`'s root** (ruling 10): `machine-readable/` →
  `.machine_readable/`. **DONE** — 108 files, one commit, on the spine; blocked
  only on a write credential at the time of writing. One repo, six verbs, and it
  is the cleanest implementation in the estate — the natural upstream once
  corrected. Low cost, no downstream blast radius (only 1 file estate-wide
  shares that root).
...
. **The 76-file hard tail** at bare-root `1-formats/contractiles/`. Unclustered, genuinely
