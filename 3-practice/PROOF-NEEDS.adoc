...
** `+2-protocols/axel/src/Tea.res+` and `+AxelApp.res+`: `+Obj.magic+` for
DOM operations
...
* `+2-protocols/avow/avow-lib/src/abi/Consent.idr+`, `+Unsubscribe.idr+`
...
* `+2-protocols/axel/src/Tea.res+` — 8+ `+Obj.magic+` calls for DOM
rendering
* Lower priority than the protocol specification proofs

==== Groove Protocol Reference

* `+2-protocols/groove/reference/groove-proxy/GrooveProxy.idr+` — Idris2
