...
These issues are now booked, in this dependency order:

* Step 1 — `hyperpolymath/standards#1020` — *closed, gate met.* GitHub Actions
  does parse a KYAML workflow: a two-arm same-commit probe gave `jobs = 1` and
  an executed-step marker on both the KYAML arm and its plain-YAML control.
  This settles the *parser* dimension only.
* Step 2 — `hyperpolymath/standards#1021` — comment-preservation proof.
* Step 3 — `hyperpolymath/standards#1022` — formatter/linter for arbitrary YAML.
...
Step 1 closing does *not* advance the scope question. Step 4 remains an owner
ruling and §5 explicitly forbids inferring scope from the probe.
