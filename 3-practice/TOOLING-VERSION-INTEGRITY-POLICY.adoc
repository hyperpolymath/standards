...
* The canonical `must` contract carries a `tooling-version-integrity`
  check (see `1-formats/contractiles/must/Mustfile`) that asserts the installed
...
(`contractile` source under `1-formats/contractiles/`, the canonical CI/Justfile
...
`1-formats/contractiles/` *source* fragments and regenerated by whoever holds the
...
* `tasks/tooling-integrity-lint.sh` — scans `.github/workflows/` for
  ...
  with `--strict`.
* `1-formats/contractiles/must/Mustfile` (canonical template) carries a default
