...
4. **Converters** - A2ML ↔ Markdown/HTML/Djot/LaTeX
...
**Go-to-definition:**
- Click on reference → Jump to ID definition

**Find references:**
- Right-click on ID → Find all uses
...
**Mapping:**
- `Section` → Markdown headings (`#`, `##`, etc.)
- `Para` → Paragraphs
- `Bullet` → Markdown lists (`-`)
- `Figure` → Markdown images
- `Opaque` → Code blocks
...
**Mapping:**
- `Section` → `<h1>`, `<h2>`, etc. with `id` attributes
- `Para` → `<p>`
- `Bullet` → `<ul>` + `<li>`
- `Figure` → `<figure>` + `<figcaption>`
- `Table` → `<table>` + `<caption>`
- `Refs` → `<section class="references">` + `<ol>`
...
**Mapping:**
- `Section` → `\section`, `\subsection`, etc. with `\label`
- `Para` → Paragraphs
- `Bullet` → `\begin{itemize}` + `\item`
- `Figure` → `\begin{figure}` + `\caption`
- `Table` → `\begin{table}` + `\caption`
- `Refs` → `\begin{enumerate}` + `\item`
...
class A2ml < Formula
  ...
  homepage "https://github.com/hyperpolymath/standards/tree/main/a2ml"
  url "https://github.com/hyperpolymath/standards/tree/main/a2ml/archive/v0.7.0.tar.gz"
  sha256 "..."
...
1. ✅ CLI with 5 commands (validate, check, format, convert, stats)
2. ✅ Format converters (Markdown, HTML, Djot, LaTeX, PlainText)
3. ✅ Interactive REPL
4. 🟡 LSP server (architecture defined, implementation in progress)
