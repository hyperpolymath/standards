// SPDX-License-Identifier: CC-BY-SA-4.0
= A2ML Web Prototype (AffineScript)

This is a minimal, non-exhaustive AFfineScript prototype that demonstrates:

* Parsing a tiny Module 0 subset
* Rendering to HTML
* Basic checked-mode validation (ID/ref resolution)

It is intentionally small and not a full implementation.

== Files

* `src/A2ml.affine` - minimal parser + renderer sketch
* `src/Demo.affine` - demo usage

== Notes

A production implementation should follow the formal grammar in `docs/GRAMMAR.adoc`
and conformance rules in `docs/CONFORMANCE.adoc`.

== Vectors

`src/VectorRunner.affine` provides a minimal vector runner for `tests/vectors/`.\n+If a matching `*.html.expected` file exists, HTML output is compared too.
