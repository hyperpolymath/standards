...
///
/// The self-validating/ directory may contain real K9 configuration files.
/// This test finds them and verifies they parse without error.
#[test]
fn e2e_dogfood_parse_k9_fixtures() {
    let crate_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    // self-validating/bindings/rust/ -> standards/self-validating/ -> standards/
    let standards_root = crate_dir
        .parent() // rust/
        .and_then(|p| p.parent()) // bindings/
        .and_then(|p| p.parent()) // self-validating/
