...
|`NOT-a-good-fit.adoc`
|N/A (documentation)
|Honest comparison of self-validating/a2ml vs alternatives (Helm, Kustomize, Terraform, etc.).
...
[source,bash]
----
podman build -t self-validating .
podman run --rm self-validating
----
...
└── examples/
    ...
    └── NOT-a-good-fit.adoc    # Deprecation analysis: self-validating vs alternatives
----
