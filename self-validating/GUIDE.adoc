...

{
  ...
  recipes = {
    deploy = "podman run --rm self-validating:latest",
...
== Container Deployment with self-validating

This section covers how self-validating wraps container deployments using the Leash
security model, selur-compose, and Nickel typed configuration.

=== When to Use self-validating for Containers

self-validating is designed for **operational deployment components** — things that
...
Use self-validating when you need:
...
=== How self-validating Wraps selur-compose

The typical workflow for a self-validating container deployment:
...
Before wrapping something in self-validating, read link:examples/NOT-a-good-fit.adoc[NOT-a-good-fit.adoc].
...
* **Do NOT** wrap plain config files in k9 pedigree. A `config.toml` is
  ...
  security benefit.
* **Do NOT** use self-validating to manage library dependencies. Libraries are
