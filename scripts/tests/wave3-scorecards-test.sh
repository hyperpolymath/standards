...
h1="$(sha256sum "$ROOT/0-canon/COMPLIANCE-DASHBOARD.adoc" | cut -d' ' -f1)"
bash "$GEN" >/dev/null 2>&1
h2="$(sha256sum "$ROOT/0-canon/COMPLIANCE-DASHBOARD.adoc" | cut -d' ' -f1)"
...
printf '\n// drift\n' >> "$ROOT/0-canon/COMPLIANCE-DASHBOARD.adoc"
