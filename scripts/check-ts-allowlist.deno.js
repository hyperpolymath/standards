...
function normalizeRepoPath(p) {
  ...
  while (((((out).length) > 0) && ((__as_strSub(out, 0, 1) === ".") || (__as_strSub(out, 0, 1) === "/")))) { out = __as_strSub(out, 1, (((out).length) - 1)); }
  return out;
}
...
function loadExemptionsFromAllowlistFile() {
  ...
  let i = 0;
  const lines_len = ((lines).length);
  while ((i < lines_len)) { const rawLine = lines[i]; const line = normalizeRepoPath(rawLine); if (((line === "") || (__as_strSub(line, 0, 1) === "#"))) { i = (i + 1); continue;  } exemptions = __as_concat(exemptions, [({ raw: line, rx: globToRegex(line) })]); i = (i + 1); }
  return exemptions;
}
...
function isExempt(p, exemptions) {
  ...
  while ((i < ex_len)) { const e = exemptions[i]; if (__as_regexMatch(target, e.rx)) { return true;  } const bare = normalizeRepoPath(e.raw); if ((target === bare)) { return true;  } if ((ends_with(bare, "/") && __as_regexMatch(target, __as_concat("^", bare)))) { return true;  } i = (i + 1); }
  return false;
}

function isTypeScriptArtifact(name) {
...
export function main() {
  ...
  while ((i < af_len)) { const f = all_files[i]; if (isTypeScriptArtifact(f)) { let skip = false; const segs = split(f, "/"); let j = 0; const segs_len = ((segs).length); while ((j < segs_len)) { const seg = segs[j]; if (((((((seg).length) > 0) && (__as_strSub(seg, 0, 1) === ".")) && (seg !== ".")) && (seg !== ".."))) { skip = true;  } j = (j + 1); } if ((!skip)) { found = __as_concat(found, [f]);  } } i = (i + 1); }
  let bad = [];
  let k = 0;
