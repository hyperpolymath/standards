...
[source,bash]
----
just registry          # writes .machine_readable/REGISTRY.a2ml + TOPOLOGY.adoc
just registry-check    # verify-only; non-zero exit on drift
----

Both `.machine_readable/REGISTRY.a2ml` and link:TOPOLOGY.adoc[TOPOLOGY.adoc] are
...
follow automatically. link:TOPOLOGY.adoc[TOPOLOGY.adoc] can therefore never
...
2. Run `just registry`.
3. Commit the regenerated `REGISTRY.a2ml` + `TOPOLOGY.adoc` alongside your change.
