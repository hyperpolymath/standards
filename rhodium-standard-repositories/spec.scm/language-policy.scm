...
(define allowed-languages
  '((affinescript . ((extensions . (".affine"))
    ...
    (pnpm         . ((extensions . ())
                     (use-case . "JS package management (tier 2)")
                     (tier . 2)
                     (preferred-instead . bun)
                     (rationale . "Only where an upstream toolchain requires a node_modules layout")
                     (note . "Tier 2: strict node_modules, content-addressable store; second reach after Bun.")))
    (npm          . ((extensions . ())
                     (use-case . "JS package management of last resort (tier 3)")
                     (tier . 3)
                     (preferred-instead . bun)
                     (rationale . "Supply chain risks")
                     (note . "Tier 3: permitted, never preferred - NOT banned. Order is bun (1) -> pnpm (2) -> npm (3) since the 2026-09-22 Deno ban vacated rank 2. It lives here, not in banned-languages, because `language-allowed?` answers from THIS list: a tier-3 fallback listed as banned reads as prohibited to every consumer. package-lock.json must still not be tracked (standards#67).")))
    (rust         . ((extensions . (".rs"))
