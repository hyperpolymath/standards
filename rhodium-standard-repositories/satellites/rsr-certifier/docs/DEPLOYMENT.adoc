...
This guide covers deploying RSR-Certified to serve as your compliance
...
[arabic]
. *Create the App*
* Go to GitHub Settings > Developer settings > GitHub Apps > New GitHub
App
* Or use manifest flow:
`+https://github.com/settings/apps/new?manifest=<base64-encoded-manifest>+`
. *Configure the App*
+
....
...
|`+RSR_LOG_LEVEL+` |Log verbosity (trace/debug/info/warn/error) |No
(default: info)

|`+RSR_PLATFORMS+` |Comma-separated list of enabled platforms |No
...
|`+GITHUB_PRIVATE_KEY+` |GitHub App private key (PEM contents) |For
GitHub
...
The service exposes: - `+GET /health+` - Returns 200 if healthy -
...
. Review logs: `+podman logs rsr-certified+`
...
[arabic]
. Verify the badge endpoint is accessible
. Check for caching issues (badges cache for 5 minutes)
