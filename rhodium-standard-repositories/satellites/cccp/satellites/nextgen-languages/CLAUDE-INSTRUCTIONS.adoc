...
This repository is the *hub* for 8 programming languages in the MAAF
ecosystem. Before making any changes to satellite repos, you MUST
...
Before working on any language repo, check `+MIGRATION-STATUS.md+`: - If
status is "`NOT SYNCED`" → sync from GitLab first - If status is
"`DIVERGED`" → consult user before making changes - If status is
"`SYNCED`" → safe to work on GitHub version

==== 3. Never Create Parallel Implementations

*WRONG:* "`I’ll implement this feature from scratch on GitHub`" *RIGHT:*
"`Let me check if this exists on GitLab first, then build on that`"

==== 4. Preserve Original Work

The GitLab repos contain months of human effort from Aug-Oct 2025. This
includes: - Design documents - White papers - Original implementations -
Research notes
...
[arabic]
. Read `+MIGRATION-STRATEGY.md+` for the overall plan
. Check `+MIGRATION-STATUS.md+` for current state
. If repo is not synced, sync it first
. Update status file after completing work
