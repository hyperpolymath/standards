...
OCaml implementation of Palimpsest License metadata tools. Supports both
native compilation and JavaScript output via Melange.
...
The native library uses `+yojson+` for JSON handling. For
Melange/browser builds, the browser module provides a JS-friendly
interface. Full Melange support may require `+melange-json+` for pure-JS
JSON handling in future versions.
