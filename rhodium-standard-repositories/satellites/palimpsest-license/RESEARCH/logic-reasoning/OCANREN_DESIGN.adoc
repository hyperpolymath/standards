...
*Status:* FIRST PASS *Purpose:* Use relational logic programming for
license reasoning
...
[arabic]
. *LKIF concepts* (legal knowledge)
. *SPDX identifiers* (standard licenses)
. *Palimpsest-specific* (emotional lineage, consent states)
...
[arabic]
. *How do we handle uncertainty?* OCanren gives definite answers. But
consent can be ambiguous. Do we need probabilistic extension?
. *How do we handle temporal aspects?* Consent can be revoked. Licenses
can be updated. How does the logic layer handle time?
. *How do we handle jurisdiction?* A license might be valid in NL but
not in US. Relational or parametric?
. *How deep does compatibility go?* Surface compatibility (terms don’t
conflict) vs. deep compatibility (philosophical alignment)?
. *Can relational programming capture emotional lineage?* Or is that
necessarily outside formal logic?
. *What happens when the logic says "`no solution`"?* Is that
"`definitely prohibited`" or "`cannot determine`"?
...
* Should we use constraint logic programming (CLP) for numeric
constraints?
