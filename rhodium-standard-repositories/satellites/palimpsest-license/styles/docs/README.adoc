...
The Palimpsest License Styling System is a comprehensive, modular SCSS
framework designed to provide consistent, accessible, and themeable
styles for the Palimpsest License project. Built with WCAG 2.3
compliance at its core, this system supports bilingual content
(Dutch/English), multiple themes, and specialised components for licence
display, metadata presentation, and compliance indicators.
...
. link:#customisation[Customisation]
...
|`+npm run scss:build:dev+` |Development build (expanded, with source
maps)
...
Source maps are generated in development mode to facilitate debugging.
They map the compiled CSS back to the original SCSS files, making it
easier to identify where styles are defined.

'''''

=== Components

The styling system includes several pre-built components specifically
designed for the Palimpsest License:
...
*Variants:* - `+.license-display--compact+` - Reduced padding -
...
See link:./COMPONENTS.md[COMPONENTS.md] for detailed component
documentation.
...
. *High Contrast Mode* (auto-detected via `+forced-colors+`)
...
[arabic]
. Create a new file in `+styles/scss/components/+`
...
[arabic, start=3]
. Import your component in `+main.scss+`:
...
See link:./CUSTOMISATION.md[CUSTOMISATION.md] for more details.
...
* *Keyboard Navigation*: Proper focus styles for all interactive
elements
...
* https://sass-lang.com/documentation[SCSS Documentation]
* https://www.w3.org/WAI/WCAG23/quickref/[WCAG 2.3 Guidelines]
...
This styling system is part of the Palimpsest License project and is
