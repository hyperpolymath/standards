...
This directory contains scripts and documentation for converting SVG
assets to various formats (TIFF, PNG, JPG) for different use cases. The
Palimpsest License project maintains multiple image formats to ensure
maximum compatibility across platforms, print media, and archival needs.
...
*Settings:* - Colour space: sRGB - Bit depth: 24-bit (8 bits per
channel) - Compression: LZW (lossless) - DPI: 300 (print quality) -
Sizes: Original dimensions from SVG

*Usage examples:* - Print media (brochures, posters) - Professional
documentation - Legal archival - Museum/library collections
...
*Settings:* - Colour space: sRGB - Bit depth: 24-bit + 8-bit alpha
(RGBA) - Compression: PNG (lossless) - Sizes: 512px, 1024px, 2048px
(width) - Background: Transparent - Optimisation: Enabled
(pngquant/oxipng)

*Usage examples:* - Website assets - GitHub README badges -
Documentation images - Social media (transparent)
...
*Settings:* - Colour space: sRGB - Bit depth: 24-bit (8 bits per
channel) - Compression: JPEG (lossy, quality 90-95%) - Sizes: 512px,
1024px, 2048px (width) - Background: White (or specified colour) -
Optimisation: Enabled (jpegoptim/mozjpeg)

*Usage examples:* - Social media posts (non-transparent) - Email
signatures - Blog posts - Documentation (where transparency not needed)
...
[arabic]
. *Always keep original SVGs:* Never delete source SVG files
. *Version control:* Commit converted assets separately from SVGs
. *Optimise for web:* Always run optimisation tools for web assets
. *Test rendering:* Verify output on multiple devices/browsers
. *Document changes:* Update CHANGELOG.md when regenerating assets
. *Accessibility:* Include alt text and ARIA labels in embed code
. *Transparency:* PNG for transparency, JPG for solid backgrounds
. *Archival:* Use TIFF for long-term storage and print

=== Further Reading

* https://imagemagick.org/index.php[ImageMagick Documentation]
* https://inkscape.org/doc/inkscape-man.html[Inkscape Command Line]
* https://sharp.pixelplumbing.com/[Sharp Documentation]
* https://css-tricks.com/snippets/html/base64-encode-of-1x1px-transparent-gif/[PNG
Optimisation Guide]
* https://www.adobe.io/open/standards/TIFF.html[TIFF Specification]
* https://en.wikipedia.org/wiki/SRGB[sRGB Colour Space]

=== Licence

These scripts and documentation are part of the Palimpsest License
project and are licensed under the same terms. See the main LICENSE.md
for details.
