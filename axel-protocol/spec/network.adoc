...
j.d.a.jewell@open.ac.uk *License*: PMPL-1.0-or-later *Status*: Optional
(Normative when implemented)
...
(schools, libraries, corporate networks) that wish to enforce AXEL
...
hostnames and IP addresses with unrestricted content, making
network-level blocking imprecise and causing collateral damage.
...
mechanism. AXEL-O (origin/edge enforcement) remains the primary
guarantee.
...
declared in the policy’s `+isolation.prefixes+` and `+scope.hostnames+`
fields.
...
* Use destination IP prefix matching from the policy’s
`+isolation.prefixes+`.
* Use DNS response filtering for `+scope.hostnames+` (when the gateway
operates a local resolver).
* Serve an informative block page via captive portal (for HTTP) or TCP
RST (for HTTPS) when blocking.
...
[arabic]
. Fetch AXEL policies for domains in their enforcement list.
. If the policy declares `+isolation.level+` >= `+L1+` and the gateway’s
...
[arabic]
. The gateway fetches the AXEL policy and extracts
`+isolation.prefixes+`.
. The gateway installs firewall rules blocking or redirecting traffic to
those prefixes.
. Rules MUST be scoped to the declared prefixes only.
...
[arabic]
. MUST fetch policies over HTTPS with TLS 1.3.
. MUST validate DNSSEC for `+_axel.<domain>+` TXT records.
. MUST respect `+cache.max_age_seconds+` and
`+cache.stale_if_error_seconds+`.
. MUST check that `+isolation.level+` is `+L1+` or `+L1-AUDITED+` before
applying network enforcement.
. SHOULD prefer `+L1-AUDITED+` content for enforcement (higher
confidence).
...
* Log enforcement actions (domain, action, timestamp) for
accountability.
...
* Use AXEL infrastructure for surveillance or content inspection beyond
the declared AXEL categories.
...
prioritizes origin enforcement (AXEL-O) over network enforcement
(AXEL-N). Managed networks provide an additional layer, not a guarantee.
...
link:origin.md[Origin Enforcement (AXEL-O)] |
